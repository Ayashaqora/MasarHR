...
| [audit-command-infrastructure-specification.md](audit-command-infrastructure-specification.md) | S04 — CommandContext, actor/correlation model, transaction boundary, audit trail model, immutability design, S03 retrofit (implemented; see the spec's Errata section for post-implementation corrections) |
| [reference-data-foundation-specification.md](reference-data-foundation-specification.md) | S05 — the `ref` schema's reference-value families, lifecycle, temporal employment-status behavior model, audit/security integration, scope-narrowing decisions |
